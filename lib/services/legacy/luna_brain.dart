...
class LunaBrain {
  ...
  Future<void> _init() async {
    await _samanthaProfile.load();
    debugPrint("🧠 LunaBrain 초기화 완료 - 친밀도: ${_samanthaProfile.attachmentLevel}");
  }
  ...
  final ConversationMemory _memory = ConversationMemory.instance; // 대화 기억
  ...
  Future<String> getResponse(String input, {String userId = "default_user"}) async {
    ...
    // 2. 유저 메시지 저장 (Hive + SQLite)
    await _memory.saveMessage(ChatMessage(
      id: '${DateTime.now().millisecondsSinceEpoch}_user',
      role: 'user',
      text: input,
      emotion: tag.toString().split('.').last,
      timestamp: DateTime.now(),
    ));
    
    // 3. 관계 업데이트 (대화 내용과 감정에 따라 친밀도 변화)
    await _updateRelationship(tag);

    // 4. 모드 결정 (업무/복잡 -> Performance, 일상/감성 -> Economy)
  ...
  Future<void> _updateRelationship(EmotionTag tag) async {
    bool isPositive = (tag == EmotionTag.seeksEmotionalConnection || 
                       tag == EmotionTag.needsReassurance || 
                       tag == EmotionTag.motivated);
    await AttachmentEngine.updateAttachment(_samanthaProfile, tag, isPositive);
  }
...
class SamanthaProfile {
  ...
  static const String _keyAttachment = 'luna_attachment_level';
  static const String _keyLastInteraction = 'luna_last_interaction';
  ...
  Future<void> save() async {
    ...
    await prefs.setDouble(_keyAttachment, attachmentLevel);
    await prefs.setString(_keyLastInteraction, lastInteractionTime.toIso8601String());
    debugPrint("💾 친밀도 저장: $attachmentLevel");
  }
  
  // 불러오기
  Future<void> load() async {
    final prefs = await SharedPreferences.getInstance();
    attachmentLevel = prefs.getDouble(_keyAttachment) ?? 10.0;
    final lastStr = prefs.getString(_keyLastInteraction);
    lastInteractionTime = lastStr != null ? DateTime.parse(lastStr) : DateTime.now();
    debugPrint("📂 친밀도 로드: $attachmentLevel");
  }
}

class AttachmentEngine {
  static Future<void> updateAttachment(SamanthaProfile profile, EmotionTag userEmotion, bool positiveInteraction) async {
    double delta = 0.0;
    // 감정에 따른 가산점
    switch (userEmotion) {
